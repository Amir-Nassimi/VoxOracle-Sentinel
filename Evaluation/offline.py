...
from Evaluation.transcribe import Transcribe
...
def main():
    ...
    parser.add_argument("--acc_thrs", type=int, required=False, default=30, help="Prediction acc threshold; default: 30%")
    parser.add_argument("--in_shape", type=str, required=False, default='128,211', help="input audio shape the model is trained with; default: 128,211")
    ...
    parser.add_argument("--window_size", type=float, required=False, default=2.1, help="The size of the window used for analysis, measured in seconds. A window of 1.9 seconds will be used to slice the audio signal into segments for processing. The choice of window size is critical; a larger window can capture more temporal context but might mix different sound events, while a smaller window may focus on instantaneous features but at the risk of losing context.")
